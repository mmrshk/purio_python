...
def fetch_products_without_score(limit: int = 20, supabase=None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        result = (
            supabase
            .table('products')
            .select('id, name, barcode, final_score')
...
def fetch_products_with_score(limit: int = 20, supabase=None) -> List[Dict[str, Any]]:
    """
    Fetch products from Supabase that already have a final_score but were not AI parsed.
    ...
    try:
        print(f"🔍 Fetching up to {limit} products with final_score from Supabase (ai_parsed = False)...")
        ...
        products = result.data
        print(f"✅ Found {len(products)} products with final_score and ai_parsed = False")
...
def fetch_ai_parsed_products(limit: int = 20, supabase=None) -> List[Dict[str, Any]]:
    """
    Fetch products whose parsed ingredients were generated by AI.
    ...
    try:
        result = (
            supabase
            .table('products')
            .select('id, name, barcode, final_score, specifications', count='exact')
            .eq("specifications->parsed_ingredients->>ai_generated", "true")
            .eq('ai_parsed', True)
            .limit(limit)
            .execute()
        )
        ...
        if hasattr(result, 'error') and result.error:
            print(f"❌ Error fetching AI-parsed products: {result.error}")
            return []

        products = result.data
        if total_count is not None:
            print(f"✅ Found {len(products)} AI-parsed products (total available: {total_count})")
        else:
            print(f"✅ Found {len(products)} AI-parsed products")

        return products

    except Exception as e:
        print(f"❌ Error fetching AI-parsed products: {str(e)}")
        return []
...
Examples:
  ...
  python process_products_without_score.py --limit 5 --dry-run
    ...
    parser.add_argument(
        '--force-ai',
        action='store_true',
        help='Force AI ingredient parsing for every product'
    )

    parser.add_argument(
        '--ai-parsed-only',
        action='store_true',
        help='Process only products whose parsed ingredients were generated by AI'
    )

    parser.add_argument(
        '--with-score',
        action='store_true',
        help='Process products that already have final_score but ai_parsed = False'
    )
    ...
    try:
        print("=" * 80)
        if args.with_score:
            print("BATCH PROCESSING PRODUCTS WITH EXISTING SCORES (ai_parsed = False)")
        elif args.ai_parsed_only:
            print("BATCH PROCESSING AI-PARSED PRODUCTS")
        else:
            print("BATCH PROCESSING PRODUCTS WITHOUT FINAL SCORE")
        print("=" * 80)
        print(f"📊 Limit: {args.limit}")
        print(f"🔒 Dry run: {args.dry_run}")
        print(f"🤖 Force AI: {args.force_ai}")
        print(f"🎯 AI parsed only: {args.ai_parsed_only}")
        print(f"📈 With existing score: {args.with_score}")
        print(f"⏰ Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("=" * 80)
        print()

        # Initialize processor with shared AI timestamp for this batch
        batch_ai_parsed_time = datetime.now().isoformat()
        processor = SingleProductProcessor(
            dry_run=args.dry_run,
            force_ai=args.force_ai,
            batch_ai_parsed_time=batch_ai_parsed_time
        )

        # Fetch products
        if args.ai_parsed_only:
            products = fetch_ai_parsed_products(limit=args.limit, supabase=processor.supabase)
        elif args.with_score:
            products = fetch_products_with_score(limit=args.limit, supabase=processor.supabase)
        else:
